...
-(UIColor*)handleColor
{
    ...
    switch (self.handleType) {
        ...
        {
            newHandleColor = [UIColor colorWithRed:255.0/255.0 green:167.0/255.0 blue:0.0/255.0 alpha:1.0];//[UIColor colorWithWhite:1.0 alpha:0.7];
            break;
        }
        ...
        case CircularSliderHandleTypeBigCircle:
        {
            if (!newHandleColor)
            {
                ...
                newHandleColor = [UIColor clearColor];//self.filledColor;
            }
            break;
        }
    }
...
-(void) drawInnerLabels:(CGContextRef)ctx
{
    ...
    if(labelsCount)
    {
        ...
        for (int i = 0; i < labelsCount; i++)
        {
            ...
            [button setFrame:CGRectMake(labelFrame.origin.x, labelFrame.origin.y, labelFrame.size.width + 20.0f, labelFrame.size.height)];
            [button setTag:i];
            [button addTarget:self action:@selector(tapedOnFanSpeedButton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(self.snapToLabels && self.innerMarkingLabels != nil)
    {
        ...
        if (selectedIndex == 0)
            ...
            selectedIndex--;
        if ([delegate respondsToSelector:@selector(getButtonClickedIndexOnSliderValueClickedIndex:)])
            [delegate getButtonClickedIndexOnSliderValueClickedIndex:selectedIndex];
    }
    /*else
    {
...
//edited by vikas
- (void)tapedOnFanSpeedButton:(UIButton *)button
{
    int tagValue = 0;
    if (button.tag <= 7)
        tagValue = (int)button.tag + 1;
    float value = 1.33 * tagValue;
    //NSLog(@" selected value %f", roundf (value * 100) / 100.0);
    if ([delegate respondsToSelector:@selector(getButtonClickedIndexValue:andClickedIndex:)])
        [delegate getButtonClickedIndexValue:(roundf (value * 100) / 100.0) andClickedIndex:(int)button.tag];
}
