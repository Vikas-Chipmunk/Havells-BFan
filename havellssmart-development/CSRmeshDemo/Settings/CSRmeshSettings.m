...
- (BleAutoConnectMode)getBleConnectMode
{
    ...
    BleAutoConnectMode connections= CSR_BLE_TWO_CONNECTIONS; //CSR_BLE_TWO_CONNECTIONS;
...
- (void)setBleListenMode:(BleListenMode)segmentValue
{
...
- (BleListenMode)getBleListenMode
{
    
    BleListenMode listenNumber=CSR_SCAN_NOTIFICATION_LISTEN_MODE;
   
    CSRSettingsEntity * settings = [[CSRDatabaseManager sharedInstance] settingsForCurrentlySelectedPlace];
    
    if (settings) {
        
        listenNumber = (BleListenMode)[[[CSRDatabaseManager sharedInstance] fetchSettingsEntity].listeningMode integerValue];
