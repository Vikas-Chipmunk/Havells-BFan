...
#pragma mark-core data basic insert delete.

- (NSManagedObject *)newManagedObjectWithClassName:(NSString *)className withValue:(NSString *)entityId forKey:(NSString *)key{
    
    NSArray *storedRecords = [[CSRDatabaseManager sharedInstance] managedObjectsForClass:className sortedByKey:key usingArrayOfIds:[NSArray arrayWithObjects: entityId, nil] inArrayOfIds:YES  forEntity:key];
    
    if (!storedRecords || [storedRecords count]==0) {
        NSManagedObject *newManagedObject = [NSEntityDescription insertNewObjectForEntityForName:className inManagedObjectContext:self.managedObjectContext];
        return newManagedObject;
    }
    else{
        return [storedRecords firstObject];
    }
}

- (NSManagedObject *)newManagedObjectWithClassName:(NSString *)className withEntityId:(NSString *)entityId {
    
    NSArray *storedRecords = [self managedObjectsForClass:className sortedByKey:@"entityId" usingArrayOfIds:[NSArray arrayWithObjects: entityId, nil] inArrayOfIds:YES  forEntity:@"entityId"];
...
- (NSManagedObject *)fetchObjectWithClassName:(NSString *)className withEntityId:(NSString *)entityId andKey:(NSString *)key{
    ...
    NSArray *storedRecords = [self managedObjectsForClass:className sortedByKey:key usingArrayOfIds:[NSArray arrayWithObjects: entityId, nil] inArrayOfIds:YES  forEntity:key];
...
- (void)deleteManagedObjectWithClassName:(NSString *)className withEntityId:(NSString *)entityId{
    ...
    NSArray *storedRecords = [self managedObjectsForClass:className sortedByKey:@"deviceId" usingArrayOfIds:[NSArray arrayWithObjects: entityId, nil] inArrayOfIds:YES  forEntity:@"deviceId"];
...
- (NSArray *)managedObjectsForClass:(NSString *)className sortedByKey:(NSString *)sortkey usingArrayOfIds:(NSArray *)idArray inArrayOfIds:(BOOL)inIds forEntity:(NSString *)entity{
    ...
    if (idArray!=nil) {
        if (inIds) {
            predicate = [NSPredicate predicateWithFormat:@"%K IN %@", entity, idArray];
        } else {
            predicate = [NSPredicate predicateWithFormat:@"NOT (%K IN %@)", entity, idArray];
        }
    }
