...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (area.devices && [area.devices count] > 1) {
        cell.numberOfAreasLabel.text = [NSString stringWithFormat:@"%lu Devices", (unsigned long)[area.devices count]];
    }
    if ([area.devices count] == 1) {
        cell.numberOfAreasLabel.text = [NSString stringWithFormat:@"%lu Device", (unsigned long)[area.devices count]];
    }
    if ([area.devices count] < 1) {
...
//    }
    ...
    if ([areaEntity.devices count] >= 1) {
        ...
        [_devicesMutableArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * stop) {
            ...
            if ([deviceEntity.appearance isEqual:@(CSRApperanceNameLight)] || [deviceEntity.appearance isEqualToNumber:@(CSRApperanceNameSensor)] || [deviceEntity.appearance isEqualToNumber:@(CSRApperanceNameSwitch)]) {
                    [self performSegueWithIdentifier:@"segmentToDevices" sender:self];
